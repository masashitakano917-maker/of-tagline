...
function limitChars(s: string, max: number) { if (!s) return s; const arr = Array.from(s); return arr.length <= max ? s : arr.slice(0, Math.max(0, max - 1)).join("") + "…"; }
function ensureMustWords(base: string, mustWords: string[]) { const miss = mustWords.filter(w => w && !base.includes(w)); return miss.length ? `${base}｜${miss.join("・")}` : base; }
function synthesizeOne({ tone, target, mustWords, charLimit }: { tone: Tone; target: string; mustWords: string[]; charLimit: number; }) {
  ...
  const pick = (n: number) => { const shuffled = [...adjs].sort(() => Math.random() - 0.5); return shuffled.slice(0, n); };
  const patterns: ((a: string[]) => string)[] = [
    (a) => `${a[0]}な暮らし、${a[1]}な邸宅。`,
  ...
  const a = pick(3); const f = patterns[Math.floor(Math.random() * patterns.length)];
  let s = f(a); if (target) s = `${target}に寄り添う、` + s; s = ensureMustWords(s, mustWords); return limitChars(s, charLimit);
}
function synthesizeCandidates({ tone, target, mustWords, charLimit, count }: { tone: Tone; target: string; mustWords: string[]; charLimit: number; count: number; }) {
  ...
  while (out.length < count && guard < count * 12) { const s = synthesizeOne({ tone, target, mustWords, charLimit }); if (!seen.has(s)) { seen.add(s); out.push(s); } guard++; }
  return out;
}
...
function synthesizeStrict({ tone, target, mustWords, charLimit, count }: { tone: Tone; target: string; mustWords: string[]; charLimit: number; count: number; }) {
  ...
  while (out.length < count && guard < count * 30) {
    ...
    const mustJoin = mustWords.join("・"); let candidate = `${mustJoin}、${base}`; if (Array.from(candidate).length > charLimit) { candidate = `${mustJoin}`; }
    candidate = limitChars(candidate, charLimit); if (includesAll(candidate, mustWords) && !out.includes(candidate)) out.push(candidate); guard++;
  }
  while (out.length < count) { const mustJoin = limitChars(mustWords.join("・"), charLimit); if (!out.includes(mustJoin)) out.push(mustJoin); else break; }
  return out.slice(0, count);
}

function useObjectUrl(file: File | null) { return useMemo(() => (file ? URL.createObjectURL(file) : ""), [file]); }
...
export default function Page() {
  ...
  const photoUrl = useObjectUrl(photo); const planUrl = useObjectUrl(plan);
  ...
  const onDrop = (e: React.DragEvent<HTMLDivElement>, which: "photo" | "plan") => {
    e.preventDefault(); e.stopPropagation(); const f = e.dataTransfer.files?.[0];
    if (f && f.type.startsWith("image/")) { which === "photo" ? setPhoto(f) : setPlan(f); }
  };

  async function handleGenerate(e: React.FormEvent) {
    e.preventDefault(); setErr(null); setStrictList([]); setFreeList([]);
    ...
    try {
      if (mode === "mock") {
        ...
        const toDataUrl = (file: File) => new Promise<string>((res, rej) => { const r = new FileReader(); r.onload = () => res(String(r.result)); r.onerror = rej; r.readAsDataURL(file); });
        const payload: any = {
          photoDataUrl: photo ? await toDataUrl(photo) : null,
          planDataUrl: plan ? await toDataUrl(plan) : null,
        ...
        if (!sList || !fList) {
          ...
          const top: string[] = []; const rest: string[] = [];
          cand.forEach(c => (mustWords.every(w => w && c.includes(w)) && top.length < strictCount ? top : rest).push(c));
          sList = Array.from(new Set(top)).slice(0, strictCount);
          fList = Array.from(new Set(rest)).slice(0, Math.max(0, count - strictCount));
        }
        // 補完
        if ((sList?.length ?? 0) < strictCount) {
          const fill = synthesizeStrict({ tone, target, mustWords, charLimit, count: strictCount - (sList?.length ?? 0) });
          sList = [...(sList ?? []), ...fill];
        }
        const needFree = Math.max(0, count - (sList?.length ?? 0));
        if ((fList?.length ?? 0) < needFree) {
          const fill = synthesizeCandidates({ tone, target, mustWords, charLimit, count: needFree - (fList?.length ?? 0) });
          fList = [...(fList ?? []), ...fill];
        }
          ...
          <div className="bg-white rounded-2xl shadow overflow-hidden h-[72vh] min-h-[560px] flex flex-col">
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button className="text-xs px-3 py-1 rounded-lg border" onClick={async()=>{ try{ await navigator.clipboard.writeText([...strictList,...freeList].join("\n")); alert("全候補をコピーしました"); }catch{} }} disabled={!strictList.length && !freeList.length}>全コピー</button>
              </div>
            </div>
                  ...
                  <ol className="space-y-2">
                    {strictList.map((txt, i) => (
                      <li key={i} className="flex items-start gap-3">
                        ...
                        <button className="text-xs px-2 py-0.5 rounded border" onClick={async()=>{ try{ await navigator.clipboard.writeText(txt);}catch{} }}>コピー</button>
                      </li>
                    ))}
                  </ol>
                )}
